...
**본 포트폴리오에 포함된 모든 프로젝트의 상세보기 링크 및 데모영상은 해당 기업과의 협업을 통해 개발된 산출물입니다.**

- 모든 프로젝트 결과물의 지적재산권은 해당 기업에 귀속됩니다.
- 본 포트폴리오는 개인의 기술 역량 및 프로젝트 경험을 소개하는 목적으로만 사용됩니다.
- 상세 내용, 소스코드, 문서 등의 무단 도용 및 상업적 이용은 법적 책임이 따를 수 있습니다.
- 본 포트폴리오의 내용은 해당 기업의 사전 동의하에 공개되었습니다.
...
- [데모영상] : https://drive.google.com/file/d/1UN6_foiEhcNy5z8c8--uaAmYJJUjZAAi/view?usp=sharing
...
- [데모영상] : https://drive.google.com/file/d/1FOjaVewZx9lQeHj78x7tpEFKIXQAxCcG/view?usp=sharing
...
- [데모영상] : https://drive.google.com/file/d/1dmU4yFK8k9ejSrBacnZFsa9kevjpW015/view?usp=sharing
...
- [데모영상] : https://drive.google.com/file/d/1gc8daoaRWn0dLTSujzCvyA7QZTKkq-C6/view?usp=sharing
...
[업무예시보기] : https://www.youtube.com/watch?v=oEsT2L3yC2E
...
성, 보이스코일 모터 진동 특성 추출 (질량, 강성, 댐핑 및 공진주파수), 임펄스 응답 추출 및 시역전 신호처리, 비접촉식 Laser Vibrometer를 이용한 진동 측정, 진동 흐름 3D 시각화, 물성치 기반 시뮬레이션 모델과 관측 모델 비교후 타당성 검증 비교

---

## 부트캠프 주요 프로젝트 - 02_SW_Bootcamp
아래는 `02_SW_Bootcamp` 폴더에 포함된 주요 팀/수업 프로젝트의 요약입니다. 각 항목은 폴더 내 README를 참고하여 핵심 목적, 기술 스택, 주요 기능을 간단히 정리하였습니다.

- **점심의 민족 (be20-1st-LunchTribe-MatzipRecommendService)**
  - 경로: `02_SW_Bootcamp/be20-1st-LunchTribe-MatzipRecommendService`
  - 설명: 부트캠프 참여생들의 리뷰 기반 가성비 맛집 추천 및 커뮤니티 플랫폼 (회원관리, 맛집 관리, 게시판, 신고/알림 등)
  - 기술: MariaDB, SQL 스크립트 중심(백엔드 DB 설계 및 쿼리), DBeaver/HeidiSQL 등 DB 관리 도구
  - 참고: DB 스키마·더미데이터 및 ERD, WBS, UML 등 문서가 포함되어 있습니다.

- **거기어때? (be20-2nd-Geogieoddae-SportsRestaurantPlatform)**
  - 경로: `02_SW_Bootcamp/be20-2nd-Geogieoddae-SportsRestaurantPlatform`
  - 설명: 스포츠 경기 관람 특화 예약 플랫폼(경기 검색, 음식점 예약, 결제, 리뷰, 점주 기능)
  - 기술: Java, Spring Boot, JPA/MyBatis, MariaDB, JWT, Swagger, Postman
  - 참고: API 명세서, ERD, DDD·화면설계 등 산출물이 포함되어 있습니다.

- **JimWhere - 창고/보관함 예약 서비스 (be20-3rd-Jimwhere-WarehouseRentalService)**
  - 경로: `02_SW_Bootcamp/be20-3rd-Jimwhere-WarehouseRentalService`
  - 설명: 기간 기반 창고·보관함 예약 및 결제·출입 관리 기능을 제공하는 B2C 플랫폼
  - 기술: Java, Spring Boot, MariaDB, Redis, Toss Payments 연동, JWT
  - 참고: 예약/결제 흐름, 멱등성 처리 사례(결제중복 방지) 등 운영 이슈와 해결 내용 포함.

- **객담 - 호텔 CRM (be20-final-Gaekdam-HotelResortCRM)**
  - 경로: `02_SW_Bootcamp/be20-final-Gaekdam-HotelResortCRM`
  - 설명: 호텔 특화 CRM으로 대시보드·리포트·고객 여정 메시징·AI 응대 보조 기능 등 포함
  - 기술: Java, Spring Boot, FastAPI(일부), MariaDB, Redis, Vue.js 프론트엔드
  - 참고: KPI 기반 대시보드·ERD·테스트 결과 등 산출물이 정리되어 있습니다.

## 대회/공모전 프로젝트 - 03_SW_Contest
`03_SW_Contest` 폴더의 대표 프로젝트 요약입니다. 각 프로젝트 README에 실행 방법과 핵심 기술이 적혀있으니, 로컬 실행이나 상세 확인 시 해당 폴더의 README를 참고하세요.

- **BlueShell2 (IoT_CoC/BlueShell2)**
  - 경로: `03_SW_Contest/IoT_CoC/BlueShell2`
  - 설명: 푸른조개 2 프로젝트(BlueShell2) — 해양 환경 관련 IoT/플랫폼 프로젝트로 재가동된 리포지토리
  - 기술 요약:
    - Backend: Spring Boot (JPA + MyBatis), MariaDB, Spring Security (JWT), Thymeleaf
    - Frontend: Vue 3 + Vite + TailwindCSS (ThreeJS, Exifr 등 사용)
  - 실행 요약 (README 참조):
    1. DB 설정: application.dev.properties에서 DB 정보 확인 및 테이블 생성
    2. Backend 실행: `BMCE(BlueShell_Marine_Cleanup_Environment)/bmce_platform`에서 `./gradlew bootRun`
    3. Frontend 실행: `BMCE(...)/bmce`에서 `npm install` 후 `npm run dev` → 기본 URL: http://localhost:5173/

- **BMCE(bmce) Frontend 템플릿**
  - 경로: `03_SW_Contest/IoT_CoC/BlueShell2/BMCE(BlueShell_Marine_Cleanup_Environment)/bmce`
  - 설명: Vue 3 + Vite 기반 프론트엔드 템플릿(개발 환경 세팅 및 빌드/배포 명령 포함)

---

원하시면 각 항목을 더 상세히(예: 주요 파일/스크립트, 실행 스크립트 블록, 주요 이슈/회고 등) 확장해 드리겠습니다.
